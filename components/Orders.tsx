...
function Orders({}: Props) {
    const limitBuyOrders: any = [
        {
            date: '11/11/22 10:33pm',
            type: 'Buy',
            price: '1200 USDC',
            amount: '2',
            total: '2400 USDC',
    ...
    const limitSellOrders: any = [
        {
            date: '11/30/22 09:31am',
            type: 'Sell',
            price: '1700 USDC',
            amount: '5',
            total: '8500 USDC',
        ...
        <div className="text-themeTextGrey flex flex-col w-full p-5 mt-5shadow-md h-96 border-b-400 min-w-min">
            <div className="flex flex-row">
                <div className="text-themeOrange">
                    Open Orders (
                    {limitBuyOrders.length + limitSellOrders.length})
                </div>
            ...
            {limitBuyOrders.map((order: any, i: number) => {
                return (
                    <div
                        key={i}
                        className="flex flex-row justify-between w-full h-10 items-center border-b-2 border-themeBorderGrey"
                    >
                        <div className="text-center w-2/12">{order.date}</div>
                        <div className="text-center w-1/12 text-themeGreen">
                            {order.type}
                        </div>
                        ...
                        <button className="text-themeOrange w-2/12">
                            Cancel
                        </button>
                    </div>
                )
            })}
            {limitSellOrders.map((order: any, i: number) => {
                return (
                    <div
                        key={i}
                        className="flex flex-row justify-between w-full h-10 items-center border-b-2 border-themeBorderGrey"
                    >
                        <div className="text-center w-2/12">{order.date}</div>
                        <div className="text-center w-1/12 text-themeRed">
                            {order.type}
                        </div>
                        <div className="text-center w-2/12">{order.price}</div>
                        <div className="text-center w-1/12">{order.amount}</div>
